...
public abstract class DocumentSelector<T> extends Document implements IDocumentSelector {
  ...
  private final IAction actionOpen = new Action(PortalConstants.c.open(), ContextImageBundle.EDIT_ICON) {
    ...
    public Permission getPermission(Object... data) {
      Permission perm = data != null && data.length == 1 && data[0] != null ? Permission.ALLOWED : Permission.READONLY;
  ...
  private final IAction actionDelete = new Action(PortalConstants.c.deletE(), ContextImageBundle.DELETE_ICON) {
    ...
    @SuppressWarnings("unchecked")
    public Permission getPermission(Object... data) {
      Permission perm = data != null && data.length == 1 && data[0] != null && canDelete((T) data[0]) ? Permission.ALLOWED : Permission.READONLY;
      return perm.add(super.getPermission(data));
    }
  };
  ...
  protected void open(T t) {
    Object parameter = PortalApp.get().mapObject2Parameter(t);
    if (parameter != null)
      PortalApp.get().openDocument(parameter);
  }
  ...
  protected void delete(final T t) {
    MessageDialog.confirm(getDisplayText(), PortalConstants.c.areYouSureToDelete(), new AsyncOkCancelAnswere() {
      @Override
      public void onOk() {
        doDelete(t);
      }
