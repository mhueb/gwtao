...
public class BorderlessButton extends Button {
  ...
  public BorderlessButton(final IAction action, int style) {
    this((style & TEXT) == 0 ? null : action.getTitle(), action.getIcon(), 0, (style & UNDERLINE) == UNDERLINE);
    init(action);
  }

  public BorderlessButton(final IAction action, int style, String altIcon) {
    this((style & TEXT) == 0 ? null : action.getTitle(), action.getIcon() == null ? altIcon : action.getIcon(), 0, (style & UNDERLINE) == UNDERLINE);
    init(action);
  }
